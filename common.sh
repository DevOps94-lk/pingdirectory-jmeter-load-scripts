...
# --- TLS client settings ---
# The .jmx samplers use trustAll=true, so no truststore is needed.
# JNDI (Java 8u181+) still enforces LDAPS hostname verification on top of trustAll;
# when pd.hostWrite is an LB IP/name that isn't in the cert SAN, every handshake
# fails with 'certificate_unknown'. Disable it for this test kit.
